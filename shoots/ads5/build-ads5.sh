#!/usr/bin/env bash
# build-ads5.sh: absurd hooks at 3-4x, coherent model behind them, winner engines, mastering standard.
set -euo pipefail
S=$SHOOT_ROOT; R=$S/takes/ads5; A2=$S/takes/ads2; B=$R/build
mkdir -p "$B"
...
for AD in orchard lantern harbor slowroad; do
  ...
  OUT=$RENDERS/ads5-$AD-final-v4.mp4
