#!/usr/bin/env bash
# build-ads4.sh: the message-first redo of the four, winner engines, mastering standard.
set -euo pipefail
S=$SHOOT_ROOT; R=$S/takes/ads4; A2=$S/takes/ads2; B=$R/build
mkdir -p "$B"
...
for AD in orchard lantern harbor slowroad; do
  ...
  OUT=$RENDERS/ads4-$AD-final-v1.mp4
