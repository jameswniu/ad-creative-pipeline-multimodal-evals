#!/usr/bin/env bash
# build-ads3.sh: the four Fonz-doctrine remakes on their winning engines, then the mastering standard.
set -euo pipefail
S=$SHOOT_ROOT; R=$S/takes/ads3; A2=$S/takes/ads2; B=$R/build
mkdir -p "$B"
...
s = open(p).read()
for ad in ("orchard", "lantern", "harbor", "slowroad"):
    tag = {"orchard": "Roasted the week it lands.", "lantern": "The page that opens the right screen.", "harbor": "One agent. One street.", "slowroad": "Go slower, see more."}[ad]
    i = s.index(tag)
    j = s.index("BED=", i)
    k = s.index(" ;;", j)
...
# the mastering standard is non-negotiable: loudnorm to -16 LUFS, 48 kHz stereo
for AD in orchard lantern harbor slowroad; do
  IN=$B/out-$AD-$AD-av/ad.mp4
  OUT=$RENDERS/ads3-$AD-final-v1.mp4
