...
PALE, DIM, EDGE, CYAN, VIOLET, AMBER = "#1f1a15", "#6f665c", "#cbbfa9", "#2f6b3f", "#b8371f", "#8c6210"
W, H = 1200, 920
...
def render():
    o = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" role="img" aria-label="System map: one loop of seven steps, board, render, closer, build, ad gates, ship gate, deliver, and the three tiers of evals that own the gates at each step.">',
         '<defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#ede5d6"/><stop offset="100%" stop-color="#f7f2ea"/></linearGradient></defs>',
    ...
    for i, (title, detail, foot, color) in enumerate(STEPS):
        ...
        o.append(f'<rect x="{x}" y="{y}" width="{cw}" height="{ch}" rx="10" fill="#fffdf9" fill-opacity=".75" stroke="{EDGE}"/>')
    ...
    for i, (name, color, lines) in enumerate(TIERS):
        x = tx0 + i * (tw + 14)
        o.append(f'<rect x="{x}" y="{ty}" width="{tw}" height="{th}" rx="10" fill="#fffdf9" fill-opacity=".75" stroke="{EDGE}"/>')
