...
The same loop as a graph, with what the figure leaves out: the fail edges, the eye, the ledger closing the loop, and which tier owns each gate, read from the stroke, solid for process, dashed for outcome, dotted for vibe, dash-dot where two tiers share the gate.

```mermaid
%%{init: {'flowchart': {'curve': 'linear', 'nodeSpacing': 28, 'rankSpacing': 34, 'padding': 8, 'subGraphTitleMargin': {'top': 6, 'bottom': 14}}, 'themeVariables': {'fontSize': '18px', 'lineColor': '#5f626a', 'edgeLabelBackground': '#ffffff', 'clusterBkg': '#f4f4f7', 'clusterBorder': '#5f626a', 'titleColor': '#18181c'}}}%%
flowchart TD
    subgraph OWN["Who owns the gate · each tier answers to its own source of truth"]
        direction LR
        P["1 Process · Board, Render, Closer, Build, Ship gate, Deliver"]
        O["2 Outcome · Ad gates"]
        V["3 Vibe · the probes inside Ad gates, the eye"]
    end
    ...
    classDef legendProcess fill:#202024,stroke:#c9a86a,stroke-width:2px,color:#e6e6ec
    classDef legendOutcome fill:#202024,stroke:#c9a86a,stroke-width:2px,stroke-dasharray:6 3,color:#e6e6ec
    ...
    class AG shared
    class V,EYE vibe
    class P legendProcess
    class O legendOutcome
    style OWN fill:#f4f4f7,stroke:#5f626a,color:#18181c
