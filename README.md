...
The same loop as a graph, top to bottom, with what the figure leaves out. Each step carries the line it must clear, from the process table above. The fail edges are re-roll, when the ad gates block delivery, and remaster, when the ship gate fails closed. The eye, the only human in the loop, takes a REVIEW off the ad gates, and an approval sends the cut on to the ship gate. The ledger closes the loop, and relabel means the thresholds are re-derived from it. Who owns a gate is in the stroke, and the table under the graph reads it.


```mermaid
%%{init: {'flowchart': {'curve': 'linear', 'nodeSpacing': 20, 'rankSpacing': 30, 'padding': 8, 'diagramPadding': 4, 'wrappingWidth': 520, 'subGraphTitleMargin': {'top': 6, 'bottom': 14}}, 'themeVariables': {'fontSize': '18px', 'lineColor': '#5f626a', 'edgeLabelBackground': '#ffffff', 'clusterBkg': '#f4f4f7', 'clusterBorder': '#5f626a', 'titleColor': '#18181c'}}}%%
flowchart TD
    subgraph RUN["The run · every request and landing ledgered"]
        direction TB
        B["Board · five checks before any spend"] --> R["Render · every request ledgered"] --> C["Closer · identity pin, jaw measured"] --> BU["Build · closer placed within 40 ms"] --> AG{{"Ad gates · captions match the speech"}} --> SG{{"Ship gate · loudness, fails closed"}} --> D["Deliver · defects withdrawn on record"]
    end
